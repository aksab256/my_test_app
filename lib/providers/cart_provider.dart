...
// =========================================================================                    
// 💡 هياكل البيانات المساعدة (Models) (تم تحديثها لإضافة imageUrl)
// =========================================================================
class CartItem {
...
class CartProvider with ChangeNotifier {
    ...
    for (var promo in promos) {
      ...
      // منطق الهدايا حسب منتج محدد
        ...
        final itemMatch = sellerData.items.firstWhere(
            ...
            orElse: () => CartItem(offerId: '', sellerId: '', sellerName: '', name: '', price: 0, unit: '', unitIndex: -1, quantity: 0, imageUrl: '')                 
        );                                      
  ...
  Future<void> addItemToCart(
    ...
    int unitIndex,
    int quantityToAdd,
    String imageUrl, // 🟢🟢 تم إضافة حقل الصورة 🟢🟢
  ) async {
  ...
  Future<void> cancelPendingCheckout() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove('checkoutOrders'); // 🛑 تم تصحيح الإكمال هنا 🛑
    notifyListeners(); 
  }
}
